...
#' @param xlim A numeric vector indicating the limits of the x-axis.
#' @param .title A list with same arguments as function \code{\link{title}}. Customization of the figure.
#'
...
plotPOD <- function(obj, model=c("auto", "simple", "full"), qLOD=95, show.ci=TRUE, show.warnings=FALSE, wmark=TRUE, unit="", xlim=NULL, .title=list(main="", xlab="Number of DNA copies", ylab="POD and ROD")){
    if(class(obj) != "pod"){
        warning("Variable 'obj' is not of class 'pod'. Did you use 'analyzeSingleLab' to generate 'obj'?")
        return(obj)
    }
    x <- obj$x
    XLIM <- range(x$X); if( !is.null(xlim) ){ XLIM <- xlim }
    YTICK <- seq(0, 1, 0.05)
    ...
    if( is.null(.title) || length(.title)==0 ){
        .title0 <- list(main="", xlab="Number of DNA copies", ylab="POD and ROD")
        sdf <- setdiff(names(.title0), names(.title))
        .title[sdf] <- .title0[sdf]
    }
    ...
    # actual plot
    plot( x=1,y=1, type="n", ylim=c(0,1), xlim=XLIM, log="x", ylab="", xlab="", yaxt="n" )
    do.call("title", .title)
